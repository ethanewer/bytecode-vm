...
void Table::clear() {
  FREE_ARRAY(TableEntry, entries, capacity);
  count = 0;
  capacity = 0;
  entries = nullptr;
}
...
bool Table::get(ObjString* key, Value* value) {
  ...
  TableEntry* entry = find_entry(entries, capacity, key);
...
TableEntry* Table::find_entry(TableEntry* entries, int capacity, ObjString* key) {
